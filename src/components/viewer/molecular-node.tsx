...
function generateAtomPositions(
  ...
  for (let i = 1; i < atomCount; i++) {
    ...
    atoms.push({
      ...
      element: surroundingElements[(i + hash) % surroundingElements.length] || 'O',
...
function generateBonds(
  atoms: { position: [number, number, number]; element: string; radius: number }[]
...
export function MolecularNode({ node, size, isHovered, isSelected }: MolecularNodeProps) {
  ...
  return (
    <group ref={groupRef}>
      ...
      {atoms.map((atom, i) => {
        ...
        return (
          <Sphere key={`atom-${i}`} args={[atom.radius, 16, 16]} position={atom.position}>
