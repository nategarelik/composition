...
interface ChatState {
  ...
  generateSuggestions: (composition: Composition, selectedNode?: CompositionNode | null) => void
}
  ...
  persist(
    (set, get) => ({
      ...
      generateSuggestions: (composition, selectedNode) => {
        const suggestions = generateContextualSuggestions(composition, selectedNode)
        set({ suggestions })
      },
    }),
    {
